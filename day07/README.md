...
### 注意事项
若引入的包为:**java.lang,则编译器默认可获取此包下的类**，**子包的类不会被导入**，不需要再显式声明。
...
### 基本概念
**多个类中存在相同属性和行为时**，将这些内容抽取到单独的一个类中，那么多**个子类就不需要再定义这些属性和行为，只要继承那个类即可**
...
* **子类不能直接访问父类中私有的的属性和方法**
* java继承只支持单继承（即一次只能继承一个父类），不允许多重继承，但允许多层继承
    * 一个子类只能有一个父类
    * 一根父类可以派生出多个子类
* 类的构造器不能被继承，因为构造器名要与类名相同
* **当父类定义了public 非默认的构造器时，必须在子类中重新定义构造器，不再提供默认的构造器了。**当父类的构造器为缺省的权限修饰符或没写，则子类中依然有默认的构造器
