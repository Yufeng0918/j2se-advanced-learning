...
**用于类, 接口和方法代码。 代码与它们能够操作的数据类型不再绑定在一起**，**可以用于多种数据类型，  不仅可以复用代码，降低耦合**。提高代码的可读性和安全性。

泛型作用
* 把集合中元素, 类, 接口的内容限制为指定的数据类型，**在实例化时指定数据类型**
* 解决元素存储的安全性问题
...
JDK 1.5之前需要自定义枚举类, JDK 1.5新增enum关键字用于定义枚举类



### 属性

* 枚举类对象的属性不应被改动，所以需要用**private final修饰**
* 枚举类的属性应该在构造器中为其赋值
